...
void UBuoyancyManager::CalculateForceToAdd()
{
	...
	for (int32 i = 0; i < WaterTestPoints.Num(); i++)
	{
		...
		staticMesh->AddForceAtLocation(gravityAmount*timescale, transformedPoint);
...
void UBuoyancyManager::OnRecievedWaveWorksDisplacement(TArray<FVector4> OutDisplacements)
{
	if (OutDisplacements.Num() > 0)
	{
		WaveWorksOutDisplacements.Add(OutDisplacements[0]);
		//for (int32 i = 0; i < OutDisplacements.Num(); i++)
		//{
		//WaveWorksOutDisplacements.Add(OutDisplacements[i]);
		//}
	}
}
...
void UBuoyancyManager::CalculateForceToAdd()
{
	...
	for (int32 i = 0; i < WaterTestPoints.Num(); i++)
	{
		FVector pointLocation = WaterTestPoints[i];
		FVector2D samplePos(pointLocation.X / 100.0f, pointLocation.Y / 100.0f);
		samplePoints.Add(samplePos);
	}
	WaveWorksComponent->SampleDisplacements(samplePoints, WaveWorksRecieveDisplacementDelegate);
	...
	for (int32 i = 0; i < WaterTestPoints.Num(); i++)
	{
		FVector currentPoint = WaterTestPoints[i];
		FVector transformedPoint = actorTransform.TransformPosition(currentPoint);
		FWavePointData waveData = ProcessWaveHeightAtPoint(currentPoint, actorTransform, i);
...
FWavePointData UBuoyancyManager::ProcessWaveHeightAtPoint(FVector pointLocation, const FTransform& actorTransform, int32 num)
{
	FWavePointData waveData;
	FVector transformedPoint = actorTransform.TransformPosition(pointLocation);
	...
	try
	{
		wwod = WaveWorksOutDisplacements[num];
	}
	catch (const std::exception&) { wwod = FVector4(0, 0, waveData.WaveHeight, 0); }
	waveData.WaveHeight = WaveWorksOutDisplacements[0].Z * 100.0f + WaveWorksComponent->SeaLevel;
